...
class SpectrogramDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return wav_data, input_dict['input_ids'], input_dict['token_type_ids'], ~(input_dict['input_ids']==0), self.label_list[index]
...
def _collate_fn(batch):    
    ...
    for x in range(len(batch)):
        ...
        mask_id = sample[3]
        target = sample[-1]
        #valence = torch.FloatTensor([sample[2]])
        #arousal = torch.FloatTensor([sample[3]])
    ...
    return seqs, input_ids, token_ids, mask_ids, targets
...
class multimodal_classifier(nn.Module):
    def __init__(self, extractor, language_model, emo_labels, dropout_prob=0.1):
        super(multimodal_classifier, self).__init__()

        self.extractor = extractor
        self.lm = AutoModel.from_pretrained(language_model)
        self.dropout = nn.Dropout(dropout_prob)
        self.output_layer = nn.Linear(1280, emo_labels)
    ...
    def forward(self, wav, input_ids, attention_mask, token_type_ids):
        ...
        output = self.output_layer(extracted_features)
        return F.softmax(output, dim=-1)
        
        #wav_output_emotion = self.wav_classifier(last_hidden_states)
        '''
        ...
        #return F.softmax(output_emotion[:, -1], dim=-1), output_valence[:, -1], output_arousal[:, -1]
        #return F.softmax(wav_output_emotion[:, -1], dim=-1), F.softmax(text_output_emotion, dim=-1)

tokenizer = AutoTokenizer.from_pretrained('klue/bert-base')
extractor = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-base-960h")
multimodal_ser_model = multimodal_classifier(extractor, 'klue/bert-base', 7)
...
for epoch_i in range(args.epochs):
    ...
    train_acc_sum = 0
    train_loss = []
    #for step, (speech, text, label, val, aro) in enumerate(train_dataloader):
    for step, (speech, text, segs, mask, label) in enumerate(train_dataloader):
        ...
        outputs = multimodal_ser_model(speech, text, segs, mask)
    ...
    with torch.no_grad():
        ...
        val_preds_list = []
        aro_preds_list = []
        for speech, text, segs, mask, label in test_dataloader:
