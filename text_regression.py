...
class SpectrogramDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return input_dict['input_ids'], input_dict['token_type_ids'], ~(input_dict['input_ids']==0), self.valence_list[index], self.arousal_list[index]
...
def _collate_fn(batch):    
    ...
    for x in range(len(batch)):
        ...
        valence = torch.FloatTensor([sample[3]])
        #print('valence', valence)
        arousal = torch.FloatTensor([sample[4]])
    ...
    return input_ids, token_ids, mask_ids, valences, arousals
...
class text_regression(nn.Module):
    def __init__(self, language_model, regress_labels, dropout_prob=0.1):
        super(text_regression, self).__init__()
...
model = text_regression('klue/bert-base', 1)
model.cuda()
######################################################
...
for epoch_i in range(args.epochs):
    ...
    for step, (text, segs, mask, val, aro) in enumerate(train_dataloader):
        optimizer.zero_grad()
        ...
        val_outputs, aro_outputs = model(text, segs, mask)
    ...
    with torch.no_grad():
        ...
        val_preds_list = []
        aro_preds_list = []
        for text, segs, mask, val, aro in test_dataloader:
