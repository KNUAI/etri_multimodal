...
parser.add_argument('--max_text_len', type=int, default=256, help='max sequence length of text')
...
class SpectrogramDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return input_dict['input_ids'], input_dict['token_type_ids'], ~(input_dict['input_ids']==0), self.label_list[index], self.valence_list[index], self.arousal_list[index]
...
def _collate_fn(batch):    
    ...
    return input_ids, token_ids, mask_ids, targets, valences, arousals
...
class text_total(nn.Module):
    def __init__(self, language_model, emo_labels, regress_labels, dropout_prob=0.1):
        super(text_total, self).__init__()
    ...
    def forward(self, input_ids, attention_mask, token_type_ids):
        ...
        output_val = self.valence_classifier(pooled_output)
        output_aro = self.arousal_classifier(pooled_output)
        output = self.output_layer(pooled_output)
        return F.softmax(output, dim=-1), output_val, output_aro
                

tokenizer = AutoTokenizer.from_pretrained('klue/bert-base')
model = text_total('klue/bert-base', 7, 1)
model.cuda()
######################################################
...
for epoch_i in range(args.epochs):
    ...
    for step, (text, segs, mask, label, val, aro) in enumerate(train_dataloader):
        optimizer.zero_grad()
        ...
        outputs, val_outputs, aro_outputs = model(text, segs, mask)
    ...
    with torch.no_grad():
        ...
        val_preds_list = []
        aro_preds_list = []
        for text, segs, mask, label, val, aro in test_dataloader:
