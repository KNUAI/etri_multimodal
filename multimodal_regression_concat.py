...
class SpectrogramDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return wav_data, input_dict['input_ids'], input_dict['token_type_ids'], ~(input_dict['input_ids']==0), self.valence_list[index], self.arousal_list[index]
...
def _collate_fn(batch):    
    ...
    for x in range(len(batch)):
        ...
        valence = torch.FloatTensor([sample[4]])
        #print('valence', valence)
        arousal = torch.FloatTensor([sample[5]])
    ...
    return seqs, input_ids, token_ids, mask_ids, valences, arousals
...
class multimodal_classifier(nn.Module):
    def __init__(self, extractor, language_model, regress_labels, dropout_prob=0.1):
        super(multimodal_classifier, self).__init__()

        self.extractor = extractor
        self.lm = AutoModel.from_pretrained(language_model)
        self.dropout = nn.Dropout(dropout_prob)
        self.valence_classifier = nn.Linear(1280, regress_labels)
        self.arousal_classifier = nn.Linear(1280, regress_labels)
        
        #self.cat_layer = nn.Linear(1280, 512)
        #self.text_classifier = nn.Linear(768, emo_labels)
        #self.wav_classifier = nn.Linear(768, emo_labels)
        #self.valence_classifier = nn.Linear(768, regress_labels)
        #self.arousal_classifier = nn.Linear(768, regress_labels)
    ...
    def forward(self, wav, input_ids, attention_mask, token_type_ids):
        ...
        #text_output_emotion = self.text_classifier(pooled_output)
        ...
        last_hidden_states = self.dropout(last_hidden_states) #B, Seq, 768
        
        # cat
        extracted_features = torch.cat([pooled_output, last_hidden_states[:, -1]], dim=-1)
        
        #output = self.cat_layer(extracted_features)
        output_val = self.valence_classifier(extracted_features)
        output_aro = self.arousal_classifier(extracted_features)
        return output_val, output_aro        

tokenizer = AutoTokenizer.from_pretrained('klue/bert-base')
extractor = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-base-960h")
multimodal_ser_model = multimodal_classifier(extractor, 'klue/bert-base', 1)
...
for epoch_i in range(args.epochs):
    ...
    total_loss = 0
    train_loss = []
    #for step, (speech, text, segs, mask, label) in enumerate(train_dataloader):
    for step, (speech, text, segs, mask, val, aro) in enumerate(train_dataloader):
        optimizer.zero_grad()
        ...
        vals = val.cuda()
        aros = aro.cuda()
        #print('vals', vals, vals.size())
        #print('aros', aros, aros.size())

        val_outputs, aro_outputs = multimodal_ser_model(speech, text, segs, mask)
    ...
    with torch.no_grad():
        ...
        val_preds_list = []
        aro_preds_list = []
        for speech, text, segs, mask, val, aro in test_dataloader:
