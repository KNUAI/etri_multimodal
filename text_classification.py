...
class SpectrogramDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return input_dict['input_ids'], input_dict['token_type_ids'], ~(input_dict['input_ids']==0), self.label_list[index]
...
def _collate_fn(batch):    
    ...
    return input_ids, token_ids, mask_ids, targets
...
class text_classificaiton(nn.Module):
    def __init__(self, language_model, emo_labels, dropout_prob=0.1):
        super(text_classificaiton, self).__init__()
...
model = text_classificaiton('klue/bert-base', 7)
model.cuda()
######################################################
...
for epoch_i in range(args.epochs):
    ...
    for step, (text, segs, mask, label) in enumerate(train_dataloader):
        optimizer.zero_grad()
    ...
    with torch.no_grad():
        ...
        val_preds_list = []
        aro_preds_list = []
        for text, segs, mask, label in test_dataloader:
