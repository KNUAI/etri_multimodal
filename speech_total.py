...
class SpectrogramDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return wav_data, self.label_list[index], self.valence_list[index], self.arousal_list[index]
...
def _collate_fn(batch):    
    ...
    return seqs, targets, valences, arousals
...
class wav2vec_classifier(nn.Module):
    def __init__(self, extractor, emo_labels, regress_labels, dropout_prob=0.1):
        super(wav2vec_classifier, self).__init__()

        self.extractor = extractor
        self.dropout = nn.Dropout(dropout_prob)
        self.classifier = nn.Linear(512, emo_labels)
        self.valence_classifier = nn.Linear(512, regress_labels)
        self.arousal_classifier = nn.Linear(512, regress_labels)
    ...
    def forward(self, wav):
        ...
        output_emotion = self.classifier(last_hidden_states) # B, Seq, 512
        output_valence = self.valence_classifier(last_hidden_states) # B, Seq, 512
        output_arousal = self.arousal_classifier(last_hidden_states) # B, Seq, 512
...
ser_model = wav2vec_classifier(extractor, args.num_labels, args.regress)
ser_model.cuda()
######################################################
...
for epoch_i in range(args.epochs):
    ...
    train_acc_sum = 0
    train_loss = []
    for step, (data, label, val, aro) in enumerate(train_dataloader):
        optimizer.zero_grad()
        ...
        outputs, val_outputs, aro_outputs = ser_model(speech)
        
        loss_multi = criterion1(outputs, labels)
        ...
        loss = loss_multi + loss_val + loss_aro
    ...
    with torch.no_grad():
        ...
        val_preds_list = []
        aro_preds_list = []
        for data, label, val, aro in test_dataloader:
